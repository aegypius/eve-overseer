...
characters.get('/:characterId/accounts', (req, res, next) => {
    ...
    co(function* () {
        try {
            let accounts = yield character.get('accounts');
...
characters.get('/:characterId/accounts/:accountKey', (req, res, next) => {
    ...
    co(function* () {
        try {
            let key = parseInt(req.params.accountKey, 10);
            let accounts = yield character.get('accounts');

            let account  = accounts.filter((item) => {
                return item.key === key;
            }).pop();

            res.status(200);
            res.json((account.transactions || []).map(transaction => {
