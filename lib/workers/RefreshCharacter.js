    ...
    *run() {
        ...
        return api.fetch('char:SkillQueue', { characterId: character.get('characterId') }).then((result) => {
            ...
            for (let id in skillqueue) {
                ...
                for (let skill of skills) {
                    if (skillMapping[skillId] !== undefined && skill.skill.toString() == skillMapping[skillId].toString()) {
                        if (!skill.queued) {
                            skill.queued = [];
                        }
...
class RefreshCharacterAccounts extends Interactor {
    *run() {
        ...
        const api = apikey.getClient();
        yield api.fetch('char:AccountBalance', { characterId: character.get('characterId') }).then((result) => {
        ...
        yield character.get('accounts').map((account) => {
            return api.fetch('char:WalletJournal', {
